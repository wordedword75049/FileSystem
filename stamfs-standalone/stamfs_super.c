...
void stamfs_write_inode (struct inode *, int);
void stamfs_dirty_inode (struct inode *);
void stamfs_put_inode (struct inode *);
void stamfs_delete_inode (struct inode *);
void stamfs_put_super (struct super_block *);
void stamfs_write_super (struct super_block *);
void stamfs_write_super_lockfs (struct super_block *);
int stamfs_statfs (struct super_block *, struct statfs *);
int stamfs_remount_fs (struct super_block *, int *, char *);
void stamfs_clear_inode (struct inode *);
void stamfs_umount_begin (struct super_block *);

 
/* The super-block operations structure. */
static struct super_operations stamfs_super_ops = {
        read_inode: stamfs_read_inode,
        write_inode: stamfs_write_inode,
        dirty_inode: stamfs_dirty_inode,
        put_inode: stamfs_put_inode,
        delete_inode: stamfs_delete_inode,
        put_super: stamfs_put_super,
        write_super: stamfs_write_super,
        write_super_lockfs: stamfs_write_super_lockfs,
        statfs: stamfs_statfs,
        remount_fs: stamfs_remount_fs,
        clear_inode: stamfs_clear_inode,
        umount_begin: stamfs_umount_begin
};
...
/*
 * Sync the given buffer to its device, and wait until it was written.
 */
void stamfs_sync_bh(struct buffer_head *bh)
{
...
void stamfs_write_inode (struct inode *ino, int opts)
{
        STAMFS_DBG(DEB_STAM, "stamfs: writing inode %ld\n", ino->i_ino);

        stamfs_inode_write_ino (ino, opts);
}

void stamfs_dirty_inode (struct inode *ino)
{
        STAMFS_DBG(DEB_STAM, "stamfs: dirting inode %ld\n", ino->i_ino);

        /* nothing to do - this is used just for debugging... */
}

void stamfs_put_inode (struct inode *ino)
{
        STAMFS_DBG(DEB_STAM,
                   "stamfs: the VFS deleted inode %ld from its cache\n",
                   ino->i_ino);
...
{
        STAMFS_DBG(DEB_STAM, "stamfs: deleting inode %ld\n", ino->i_ino);
        ...
        ino->i_size = 0;
        if (ino->i_blocks) {
                STAMFS_DBG(DEB_STAM, "stamfs: truncating, #blocks = %ld\n", ino->i_blocks);
                stamfs_inode_truncate(ino);
        }

        /* free the block index and the inode's block. */
        stamfs_inode_free_inode(ino);
...
void stamfs_write_super_lockfs (struct super_block *sb)
{
        STAMFS_DBG(DEB_STAM,
                   "stamfs: writing locked superblock, dev='%d:%d'\n",
                   major(sb->s_dev), minor(sb->s_dev));

        /* nothing to do - see comment in stamfs_write_super(). */
        sb->s_dirt = 0;
}
...
int stamfs_remount_fs (struct super_block *sb, int *dummy, char *opts)
{
        /* TODO */
        STAMFS_DBG(DEB_STAM,
                   "stamfs: the VFS remounted the FS, dev='%d:%d'\n",
                   major(sb->s_dev), minor(sb->s_dev));
...
void stamfs_umount_begin (struct super_block *sb)
{
        STAMFS_DBG(DEB_STAM,
                   "stamfs: the VFS begun umount for the FS, dev='%d:%d'\n",
                   major(sb->s_dev), minor(sb->s_dev));

        /* nothing to do - this function is just to ease debugging. */
}
